...
fh = logging.FileHandler('plstream-fasttext.log', mode='w')
formatter = logging.Formatter('plstream-fasttext:%(thread)d %(lineno)d: %(levelname)s: %(asctime)s %(message)s',
...
class for_output(MapFunction):
    ...
    def map(self, value):
        self.logFile('plstream-fasttext.log', value[0]+'\n')
        return str(value[1])
...
class unsupervised_OSA(MapFunction):
    ...
    def map(self, tweet):
        ...
        # self.logFile('plstream-fasttext.log',str(self)+'\n')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run plstream-fasttext in two modes, labelling and accuracy. Accuracy\
     mode is default')
    parser.add_argument('-l', dest='mode', action='store_const', default='ACC', const='LABEL',
                        help='Generate label(default: print accuracy)')
    args = parser.parse_args()
    MODE = args.mode
    logging.basicConfig(filename='plstream-fasttext.log')
    ...
    parallelism = 4
    # the labels of dataset are only used for accuracy computation, since plstream-fasttext is unsupervised
    ...
    for j in range(1):
        ...
        ds.map(unsupervised_OSA()).set_parallelism(parallelism) \
            ...
            .add_sink(StreamingFileSink  # .set_parallelism(2)
                      .for_row_format('./output_FastText', Encoder.simple_string_encoder())
                      .build())
        env.execute("fasttext_job")
