...
class Trainer:
    ...
    def __init__(self, vectors, labels, input_size, init, test_size=0.2, batch_size=16,
        ...
        Args:
            ...
            downsample (bool, optional): Flag indicating whether to downsample to balance classes.
                                        Defaults to False.
        """
        # Determine if we have a GPU available for training
        ...
        test_data = SentimentDataset(x_val, y_val)
        ...
        if init:
            self.torch_model = Classifier(input_size=input_size, hidden_size=hidden_size)
        else:
            self.torch_model = load_torch_model('ssl-clf.pth')
        self.torch_model.to(self.device)
        self.criterion = nn.BCELoss()
        self.optimizer = torch.optim.Adam(params=self.torch_model.parameters(), lr=learning_rate)
    ...
    def fit(self, epochs):
        """
        Train the Simple ANN for specified number of epochs and stores the best model based on
        validation loss.
        ...
        for epoch in range(epochs):
            ...
            for vecs, labels in self.train_loader:
                ...
                train_acc += binary_acc(outputs, labels).item()
            ...
            with torch.no_grad():
                ...
                for vecs, labels in self.test_loader:
                    ...
                    val_loss += loss.item()
                    val_acc += binary_acc(outputs, labels).item()
