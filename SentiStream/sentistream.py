...
class unsupervised_OSA(MapFunction):
    ...
    def update_model(self, new_sentences):
        ...
        for word in call_model.wv.index_to_key:
            if word not in self.vocabulary:  # new words
                self.LRU_index.insert(0, self.max_index + 1)
                self.max_index += 1
            else:  # duplicate words
                old_word_index = self.vocabulary[word] + 1
                self.LRU_index.remove(old_word_index)
                self.LRU_index.insert(0, old_word_index)
    ...
    def eval(self, tweets, model):
        ...
        return str(ans)# + str(' ') + str(self.time_finish) #+ str(self.trend) #+ '@'
...
if __name__ == '__main__':
    ...
    ds.map(unsupervised_OSA()).set_parallelism(4)\
