...
def test_sentistream(percent, batch_size, lr, test_size, use_pretrained=True, data_set='data'):
    """
    ...
    """
    config.DATA = f'data/{data_set}_{percent}_percent.csv'
    config.TRAIN_DATA = f'data/{data_set}_train_{percent}_percent.csv'

    if use_pretrained:
        load_pretrained_models(percent, data_set)
    else:
        init_train(batch_size=batch_size, lr=lr, test_size=test_size)

    time, us_acc, us_f1, ss_acc, ss_f1, senti_acc, senti_f1 = stream_process()
    print(f'Elapsed Time for model trained on {percent} percent: ', time)

    with open(f'outputs/output_{data_set}_{percent}.csv', 'w', newline='', encoding='utf-8') as file:
