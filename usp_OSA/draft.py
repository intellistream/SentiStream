...
if __name__ == '__main__':
    ...
    collectors = [['good','number','today','fulton'],['sad','weather']]
vocabulary = model.wv.key_to_index
LRU_index = [0,1,2,3,4,5,6,7]
size_of_vocab = len(vocabulary)
length_LRU = len(LRU_index)
new_words = []
word_to_call = []

# first to check if min_count is satisfied
for sentence in collectors:
    for word in sentence:
        if word not in vocabulary: # new word
            new_words.append(word)
        else:
            word_to_call.append(word)
corresponding_vectors = redis.call(index_to_call)

def build_weight_matrix(corresponding_vectors,vector_size,size_of_vocabulary,new_words,vocabulary,index_to_call):
    ...
    for index in index_to_call:
        weight_matrix[index] = corresponding_vectors[index] 
    weight_matrix = np.vstack([weight_matrix, np.zeros((len(new_words), vector_size), dtype=REAL)])
    return weight_matrix

model = Word2Vec(common_texts, vector_size=300, window=2, min_count=3,hs=0,negative = 5)

    # update vectors to Redis
duplicate_words = []
duplicate_word_new_vector = []
new_words = []
new_vector = []
for i in range(len(duplicate_words)):
    r.getset(duplicate_words[i],duplicate_word_new_vector[i])
for j in range(len(new_words)):
    r.set(duplicate_words[j],duplicate_word_new_vector[j])

# call vectors from Redis
# first call: vocabulary
redis_vocabulary = r.get('vocab')
LRU_index = r.get('lru')
...
# str to list
vec = list(map(lambda x: int(x),r.get('good')
                                 .decode()
                                 .replace('[','')
                                 .replace(']','')
                                 .split(',')))
